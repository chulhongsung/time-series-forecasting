...
cate_input = np.load("./data/cate_input_k30_tau70.npy")
real_input = np.load("./data/real_input_k30_tau70.npy")
future_input = np.load("./data/future_k30_tau70.npy")
target = np.load("./data/target_k30_tau70.npy")
...
class TFT(K.models.Model):
    def __init__(self, d_embedding, num_rv, d_model, d_param, dr, cat_dim, future_dim, tau, num_heads):
...
tft_model = TFT(d_embedding=args.d_emb, num_rv=17, d_model=args.d_model, d_param=11, dr=args.dr, cat_dim=[4, 4, 12, 31], future_dim=2, tau=args.tau, num_heads=args.num_h)
...
tft_model.save_weights("./save_weights/tft_model_" + datetime.date.today().strftime("%Y%m%d") + ".h5")
