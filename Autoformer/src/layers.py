...
class SeriesDecomp(K.layers.Layer):
    def __init__(self, kernel_size):
        ...
        self.moving_avg = K.layers.AveragePooling1D(pool_size=kernel_size, padding='same')
...
class AutoCorrelation(K.layers.Layer):
    ...
    def time_delay_agg(self, q, k, v):
        ...
        init_index = tf.repeat(tf.expand_dims(tf.expand_dims(tf.expand_dims(tf.range(10), axis=0), axis=0), axis=0), [batch_size, self.num_heads, self.depth, 1])

        top_k = int(2 * math.log(timestpes))
...
class DecoderLayer(K.layers.Layer):
    ...
    def call(self, x, x_en, init_trend):
        ...
        x, trend2 = self.series_decomp2(self.autocorrelation2(x, x_en, x) + x)
        x, trend3 = self.series_decomp3(self.dropout(self.dense(x)) + x)
